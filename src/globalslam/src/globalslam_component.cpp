...
: Node("gslam", options)
{
  ...
  client_control_run_ = this->create_client<interfaces::srv::SimpleServ>("control_run_service"); 
  // Set publishers
  // Set publishers
  ...
  init_gmap_flag = false;
  init_cloop_flag = false;
  rechome_flag = false;
...
void Gslam::GMAP_LOOP()
  {
    ...
    while(gmap_thread_loop_run == true)  // thread main loop
    {
        ...
        //-- check if a reset command has been recieved
        mutex_init_gmap.lock();
          if(init_gmap_flag == true)
          { 
            init_gmap_flag = false;
            gmap.InitGmap();
          }
        mutex_init_gmap.unlock();
...
void Gslam::CLOOP_LOOP()
{
  ...
  while(cloop_thread_loop_run == true)  // thread main loop
    {   
      ...
      //-- check if a reset command has been recieved
        mutex_init_cloop.lock();
          if(init_cloop_flag == true)
          { 
            init_cloop_flag = false;
            cloop.CLOOP_init();
          }
        mutex_init_cloop.unlock();
      ...
      if(new_kf_cl == true)
      {
          ...
          mutex_rechome.lock();             
            if(rechome_flag == true){
              cloop.Set_n_kf_rechome(); 
              rechome_flag = false; 
            }                    
          mutex_rechome.unlock();

          mutex_activate_CL.lock();
            if(activate_CL_flag == true)
            {
              activate_CL_flag = false;
              cloop.Set_Activate_Deactivate_CL(true);
            }  
          mutex_activate_CL.unlock();
          ...
          if(cloop.Get_close_loop_state() == true)
          {
            ...
            // -- send close loop signal to control component 
...
void Gslam::Publish_gmap_data()
{
  ...
  int j = PAR.GM_factor_anchors_sent_to_plot;
  
  if(Gmap.AnchorsDATA.size() > 100)j= 10;

  for(int i = 0; i < Gmap.AnchorsDATA.size(); i = i +j)
  { 
...
void Gslam::Kf_cl_callback(const interfaces::msg::Kf & msg) 
  {
    ...
    */
    ...
    kf.radius_uncertainty = msg.radius_uncertainty;

    //cout <<  msg.radius_uncertainty << endl;     
...
void Gslam::Handle_globalslam_run_service(const std::shared_ptr<interfaces::srv::SimpleServ::Request> request,std::shared_ptr<interfaces::srv::SimpleServ::Response> response) 
    {       
      ...
      if (request->cmd == 'i') // re-initialize gmap and cloop
      { 
