...
: Node("PLOTscene", options)
{
  ...
  sub_Gps_ = this->create_subscription<geometry_msgs::msg::PointStamped>("GT_topic", 10, std::bind(&PLOTscene::GT_callback, this, _1));
  // plot thread
  viewer_active = true;
  new_robot_pose_flag = false;
  new_clear_scene = false;
  init_scene = false;
  slam_start_flag = false;
  robot_uncertainty = 0;
...
void PLOTscene::draw_drone()
{
  ...
  // Create an ellipse widget
  cv::Vec3d robot_p = robot_pose.translation();
  cv::viz::WCircle Uncertanty(robot_uncertainty,robot_p, cv::Vec3d(0.0,0.0,1.0), 0.01  , cv::viz::Color::green());
  
  viewer.showWidget("Ucircle", Uncertanty);
}
//-------------------------------------------
void PLOTscene::draw_localslam_trajectory()
{
        ...
        if(Draw_local_slam_trajectory && LocalSlam_trajectory.size() > 1)
        {
        ...
        if(Draw_actual_trayectory && Actual_trajectory.size() > 1)
        {
          int i = Actual_trajectory.size()-1;
          string id = to_string(i);
          viz::WLine segment(Actual_trajectory[i-1], Actual_trajectory[i]);
          segment.setRenderingProperty(viz::LINE_WIDTH, 3.0);
          segment.setRenderingProperty(viz::OPACITY,1.0);
          segment.setColor(viz::Color::yellow());
          viewer.showWidget(id, segment);
        }
...
//---------------------------------------------
void PLOTscene::take_screenshot()
{ 
  
  // take screenshot
    string file_name = "ss" + to_string(n_ss) + ".png";
...
void  PLOTscene::lslam_data_callback(const interfaces::msg::Lslam & msg) 
{
  ...
  */  
  ...
  mutex_get_state.lock();
    ...
    robot_uncertainty = msg.radius_uncertainty;
...
//-----------------------------------------------------------
//-----------------------------------------------------------------------
  void PLOTscene::GT_callback(const geometry_msgs::msg::PointStamped & msg)
  { 
    ...
    { 
      if (init == false)
      {
        ...
        mutex_webots_state.lock();
          cv::Vec3f  t_r;
          t_r(0) = x - x0;//cv::Mat pt(1, 3, CV_64FC3); 
          t_r(1) = y - y0;
          t_r(2) = z - z0;
          Actual_trajectory.push_back(t_r);      
...
{  
   ...
   this->declare_parameter<bool>("Draw_actual_trayectory",true);   
   this->declare_parameter<bool>("Plot_actual_cam_pos",true);
   this->declare_parameter<bool>("Plot_local_map",true);
   this->declare_parameter<bool>("Plot_global_map",true);
   ...
   this->get_parameter("Draw_actual_trayectory",Draw_actual_trayectory);
   this->get_parameter("Plot_actual_cam_pos",Plot_actual_cam_pos);
   this->get_parameter("Plot_local_map",Plot_local_map);
   this->get_parameter("Plot_global_map",Plot_global_map);
